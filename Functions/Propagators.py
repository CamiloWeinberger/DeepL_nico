...
def Prop2VanillaPyrWFS_torch(phaseMap,wfs):
    ...
    # include cropping
    r1 = np.linspace(int(sx/8)+1,int(3*sx/8),int(sx/4))
    r2 = np.linspace(int(sx/8)+1,int(3*sx/8),int(sx/4))+int(sx/2)
    I1 = I4Q[:,:,r1,:][:,:,:,r1]
    I2 = I4Q[:,:,r1,:][:,:,:,r2]
    I3 = I4Q[:,:,r2,:][:,:,:,r1]
    I4 = I4Q[:,:,r2,:][:,:,:,r2]
    I4Q = torch.cat((torch.cat((I1,I2),3),torch.cat((I3,I4),3)),2)
    return(I4Q)
